   ...
   animate(){
       ...
       this.oiseau.forEach((bird  , birdIndex)=> {
           ...
           if (bird.estCollisionAvec(this.arc)) {
               ...
               document.getElementById("life-" + ((this.nbVie)+1).toString()).style.display='none';
               return;
           }
       ...
       tirerFleches(){
           if (!this.#estPause && this.nbFleches > 0) {
               this.fleche.push(new Fleche(this.arc.x + this.arc.width / 2 - Game.F_WIDTH / 2, this.arc.y, Game.F_WIDTH, Game.F_HEIGHT));
               this.nbFleches--;
               document.getElementById("nbArrows").textContent = this.nbFleches;
           }
       }
       ...
       startAndStop()
       {
           if (this.raf === null) {
               ...
               this.interCarquois = setInterval(() => (Math.round(Math.random())) === 1 ? this.carquois = new Carquois(this.canvas.width) : null, 1500);
               this.interOiseau = setInterval(() => Math.random() > 0.25 ? this.ajoutOiseau() : null, 1000);
               this.raf = window.requestAnimationFrame(this.animate.bind(this));
       ...
       keyDownActionHandler(event)
       {
           switch (event.key) {
               ...
               case "Space":
               case " " :
                   if (!event.repeat) this.fleche.push(new Fleche(this.arc.x + this.arc.width / 2 - Fleche.F_WIDTH / 2, this.arc.y));
